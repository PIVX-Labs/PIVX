    ...
    def run_test(self):
        ...
        for i in range(len(self.nodes)):
            if (i in [m.idx for m in members]):
                assert_equal(True, self.nodes[i].quorum_sigs_cmd("sign", 100, id, msgHash))
            else:
                assert_equal(False, self.nodes[i].quorum_sigs_cmd("sign", 100, id, msgHash))
        ...
        for i in range(len(self.nodes)):
            assert_equal(False, self.nodes[i].quorum_sigs_cmd("hasrecsig", 100, id, msgHashConflict))
        ...
        for i in range(len(self.nodes)):
            if (i in [m.idx for m in members]):
                if firstSigner:
                    assert_equal(True, self.nodes[i].quorum_sigs_cmd("sign", 100, id, msgHashConflict))
                    firstSigner = False
                else:
                    assert_equal(True, self.nodes[i].quorum_sigs_cmd("sign", 100, id, msgHash))
            else:
